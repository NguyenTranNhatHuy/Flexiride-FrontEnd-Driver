...
export const PickupProgressScreen = ({ route, navigation }) => {
  ...
  const fetchRides = async () => {
    setLoading(true);
    try {
      const response = await getCustomerStatusPickup(rideInfor._id, authState.token);
        ...
        <View style={styles.detailContainer}>
          <View style={styles.infoContainer}>
            ...
            <Text style={styles.detailText}>
              Trạng thái: <Text style={[styles.highlightText, { color: getStatusColor(rideStatus) }]}>{rideStatus}</Text>
            </Text>
          ...
          {rideStatus === 'completed' && (
            <TouchableOpacity style={styles.routeButton} onPress={() => navigation.navigate('ManageDriverRides')}>
